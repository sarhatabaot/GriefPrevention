...
public class SetAccruedClaimBlocksCommand extends GPBaseCommand {
	...
	public void onSet(final CommandSender player, final String name, final int amount){
		...
		if(player != null) GriefPrevention.addLogEntry(player.getName() + " set " + targetPlayer.getName() + "'s accrued claim blocks to " + amount + ".", CustomLogEntryTypes.ADMIN_ACTIVITY);
