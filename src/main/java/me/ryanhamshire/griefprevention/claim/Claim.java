...
public class Claim {
	...
	private Map<String, ClaimPermission> playerIDToClaimPermissionMap = new HashMap<>();
	...
	private boolean hasExplicitPermission(Player player, ClaimPermission level) {
		...
		for (final String identifier : keys) {
	...
	public void getPermissions(List<String> builders, List<String> containers, List<String> accessors, List<String> managers) {
		...
		for (final Map.Entry<String, ClaimPermission> entry : this.playerIDToClaimPermissionMap.entrySet()) {
	...
	public String allowMoreEntities(boolean remove) {
		...
		int totalEntities = 0;
		ArrayList<Chunk> chunks = new ArrayList<>(this.getChunks());
