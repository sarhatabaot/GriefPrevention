...
public class Visualization {
	public List<VisualizationElement> elements = new ArrayList<>();
	...
	public static void apply(Player player, Visualization visualization) {
		...
		if (player.isOnline() && !visualization.elements.isEmpty() && visualization.elements.get(0).getLocation().getWorld().equals(player.getWorld())) {
	...
	private void addClaimElements(Claim claim, int height, VisualizationType visualizationType, Location locality) {
		...
		for (int i = 0; i < newElements.size(); i++) {
			...
			if (!claim.contains(element.getLocation(), true, false)) {
